...
from flow360.component.flow360_params import (
    ...
    MeshBoundary,
    TimeStepping,
    ActuatorDisk, ForcePerArea,
    SlidingInterface,
    NavierStokesSolver,
    TurbulenceModelSolver,
    NoSlipWall, SlipWall, FreestreamBoundary, IsothermalWall, SubsonicOutflowPressure, SubsonicOutflowMach, SubsonicInflow, SlidingInterfaceBoundary, WallFunction, MassInflow, MassOutflow
)
...
def test_boundary_types():
    ...
    assert SubsonicInflow(totalPressureRatio=1, totalTemperatureRatio=1, rampSteps=10).type == "SubsonicInflow"
...
def test_sliding_interface():
    ...
    assert si_rpm.to_flow360_json(mesh_unit_length=0.01, C_inf=1) == si_omega.to_flow360_json(mesh_unit_length=0.01, C_inf=1)
    ...
    assertions.assertAlmostEqual(json.loads(params.to_flow360_json())['slidingInterfaces'][0]['omegaDegrees'], 2.938e-06)
...
def test_time_stepping():
    ...
    assertions.assertAlmostEqual(json.loads(params.to_flow360_json())['timeStepping']['timeStepSize'], 0.1)
    to_file_from_file_test(ts)
...
def test_turbulence_solver():
    ...
    with pytest.raises(pd.ValidationError):
        ts = TurbulenceModelSolver(
            model_type='SA',
            grid_size_for_LES='other_option'
        )        
