...
def _validate_cht_has_heat_transfer_zone(values):
    ...
    boundaries = values.get("boundaries")
    if boundaries is not None:
        for boundary_prop in dict(boundaries).values():
...
def _validate_cht_no_heat_transfer_zone(values):
    ...
    if time_stepping is not None and isinstance(time_stepping, UnsteadyTimeStepping):
        for volume_prop in dict(volume_zones).values():
...
def _check_equation_eval_frequency_for_unsteady_simulations(values):
    ...
    if isinstance(time_stepping, UnsteadyTimeStepping):
        ...
        turbulence_default_eq_eval_freq = TurbulenceModelSolver().equation_eval_frequency
        turbulence_curr_eq_eval_freq = turbulence_model_solver.equation_eval_frequency
        if max_pseudo_steps < turbulence_curr_eq_eval_freq:
            raise ValueError(
                f"'equation evaluation frequency' in turbulence_model_solver  \
                (default: {turbulence_default_eq_eval_freq}) is greater than max_pseudo_steps."
            )

        transition_default_eq_eval_freq = TransitionModelSolver().equation_eval_frequency
        transition_curr_eq_eval_freq = transition_model_solver.equation_eval_frequency
        if max_pseudo_steps < transition_curr_eq_eval_freq:
            raise ValueError(
                f"'equation evaluation frequency' in transition_model_solver  \
                (default: {transition_default_eq_eval_freq}) is greater than max_pseudo_steps."
            )
    return values
