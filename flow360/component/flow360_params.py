...
from .types import Axis, Coordinate, PositiveFloat, MomentLengthType, BoundaryVelocityType, PositiveInt, NonNegativeInt, Omega, Velocity, TimeStep
...
class Boundary(ABC, Flow360BaseModel):
    ...
    type: str
    name: Optional[str] = pd.Field(None, title="Name", description="Optional unique name for boundary.")
...
BoundaryType = Union[NoSlipWall, SlipWall, FreestreamBoundary, IsothermalWall, SubsonicOutflowPressure, SubsonicOutflowMach, SubsonicInflow, SlidingInterfaceBoundary, WallFunction, MassInflow, MassOutflow]
...
class ForcePerArea(Flow360BaseModel):
    ...
    thrust : Axis
        Force per area in the axial direction, positive means the axial force follows the same direction as axisThrust. It is non-dimensional: trustPerArea[SI=N/m2]/rho_inf/C_inf^2
    
    circumferential : PositiveFloat
        Force per area in the circumferential direction, positive means the circumferential force follows the same direction as axisThrust with the right hand rule. It is non-dimensional: circumferentialForcePerArea[SI=N/m2]/rho_inf/C_inf^2
    ...
    def check_len(cls, values):
        radius, thrust, circumferential = values.get('radius'), values.get('thrust'),  values.get('circumferential')
        if len(radius) != len(thrust) or len(radius) != len(circumferential):
            raise ValidationError(f'length of radius, thrust, circumferential must be the same, but got: len(radius)={len(radius)}, len(thrust)={len(thrust)} len(circumferential)={len(circumferential)}')
...
class SlidingInterface(Flow360BaseModel):
    ...
    omega : Union[float, Omega], optional
        Rotating speed without or with unit, eg. 1.0, (1.0, 'rad/s'), (1.0, 'deg/s'), (1.0, 'non-dim'). If no unit provided, non-dimensional is assumed.
    ...
    @export_to_flow360
    def to_flow360_json(self, mesh_unit_length=None, C_inf=None, return_json: bool=True):
    ...
    def perform_non_dimensionalisation(self, mesh_unit_length, C_inf):
        if self.omega:
            if isinstance(self.omega, float):
                ...
                self.omega_radians = self.omega.v * get_time_non_dim_unit(mesh_unit_length, C_inf, extra_msg='when using rad/s unit for omega.')
            elif self.omega.unit=='deg/s':
                self.omega_degrees = self.omega.v * get_time_non_dim_unit(mesh_unit_length, C_inf, extra_msg='when using deg/s unit for omega.')
            elif self.omega.unit=='non-dim':
                self.omega_degrees = self.omega.v

        if self.rpm:
            omega = self.rpm * (2 * math.pi) / 60
            self.omega_radians = omega * get_time_non_dim_unit(mesh_unit_length, C_inf, 'when using rpm.')


    class Config(Flow360BaseModel.Config):
        exclude_on_flow360_export = ['omega', 'rpm']
        require_one_of = ['omega', 'rpm', 'omega_radians', 'omega_degrees', 'theta_radians', 'theta_degrees', 'is_dynamic']
...
class MeshSlidingInterface(Flow360BaseModel):
    ...
    def from_case_sliding_interface(cls, si: SlidingInterface):
        return cls(stationary_patches=si.stationary_patches, rotating_patches=si.rotating_patches, axis=si.axis, center=si.center)
...
class TimeStepping(Flow360BaseModel):
    ...
    time_step_size: Optional[Union[pd.confloat(gt=0, allow_inf_nan=False), TimeStep, Literal["inf"]]] = pd.Field(alias="timeStepSize", default='inf')
    CFL: Optional[TimeSteppingCFL] = pd.Field()

    @classmethod
    def default_steady(cls):
        return cls(physical_steps=1, time_step_size="inf", max_pseudo_steps=2000, CFL=TimeSteppingCFL.default_steady())

    @classmethod
    def default_unsteady(cls, physical_steps, time_step_size):
        return cls(physical_steps=physical_steps, time_step_size=time_step_size, max_pseudo_steps=40, CFL=TimeSteppingCFL.default_unsteady())
    ...
    def perform_non_dimensionalisation(self, mesh_unit_length, C_inf):
        if isinstance(self.time_step_size, TimeStep):
            if self.time_step_size.is_second():
                self.time_step_size = self.time_step_size.v / get_time_non_dim_unit(mesh_unit_length, C_inf, extra_msg='when using time_step_size in seconds.')
...
class GenericBoundaryWrapper(Flow360BaseModel):
    v: BoundaryType
...
class Boundaries(Flow360BaseModel):
    ...
    <boundary_name> : BoundaryType
        Supported boundary types: Union[NoSlipWall, SlipWall, FreestreamBoundary, IsothermalWall, SubsonicOutflowPressure, SubsonicOutflowMach, SubsonicInflow, SlidingInterfaceBoundary, WallFunction, MassInflow, MassOutflow]
    ...
    def validate_boundary(cls, values):
        for key, v in values.items():
            try:
                values[key] = GenericBoundaryWrapper(v=v).v
            except:
                raise ValidationError(f'{v} (type={type(v)}) is not any of supported boundary types.')
        return values
...
class Geometry(Flow360BaseModel):
    ...
    def mesh_unit_length_in_meter(self, unit):
        in_meter = {
            'm': 1,
            'cm': 0.01,
            'mm': 0.001,
...
class Freestream(Flow360BaseModel):
    ...
    @classmethod
    def _speed_of_sound_from_temperature(self, T):
    ...
    @classmethod
    def from_speed(cls, speed: Union[Velocity, PositiveFloat]=None, temperature: PositiveFloat=288.15, density: PositiveFloat=1.225, **kwargs) -> Freestream:
        assert speed
    ...
    @export_to_flow360
    def to_flow360_json(self, mesh_unit_length=None, return_json: bool=True):
        if self.Reynolds is None and self.mu_ref is None:
            ...
            self.mu_ref = viscosity / (self.density * self.get_C_inf()) / get_length_non_dim_unit(mesh_unit_length)
...
class TurbulenceModelSolver(GenericSolverSettings):
    ...
    DDES : bool, optional
        Enables Delayed Detached Eddy Simulation. Supported for both SpalartAllmaras and kOmegaSST turbulence models, with and without AmplificationFactorTransport transition model enabled.
    ...
    model_type: TurbulenceModelModelType = pd.Field(alias="modelType", default=TurbulenceModelModelType.SA)
...
class Flow360Params(Flow360BaseModel):
    ...
    def to_flow360_json(self):
        ...
        if not self.turbulence_model_solver:
            ...
            # # self.__setattr__('turbulence_model_solver', TurbulenceModelSolver())
            # print(self.turbulence_model_solver)
