...
from typing import Literal, Any, Optional, cast, Callable, Union, List
...
class Flow360BaseModel(BaseModel):
    ...
    @classmethod
    def dict_from_file(cls, filename: str, group_path: str = None) -> dict:
    ...
    def append(self, params: Flow360BaseModel, overwrite: bool=False):
        ...
        for key, value in additional_config.items():
            if self.__getattribute__(key) and not overwrite:
                log.warning(f'"{key}" already exist in the original model, skipping. Use overwrite=True to overwrite values.')
                continue
